...
const buildSnippet = (prefix, params) => {
  ...
  return `${prefix}_path(${args.join(', ')})\${${params.length + 1}}`;
};
...
export default class AutocompleteProvider {
  ...
  getSuggestions({ bufferPosition, editor, prefix }) {
    ...
    return this.buildSuggestions({
      ...
      prefix: prefix.replace(/_?(?:path|url)$/, ''),
      editor,
    });
  }
  ...
  buildSuggestions({ editor, prefix }) {
    ...
    this.routes.getAll().forEach(({ url, actions, params, controller }, routePrefix) => {
      const score = fuzzaldrinPlus.score(routePrefix, prefix);
      if (score === 0) return;
      const controllerScore = matchScore(getCurrentController(editor), controller);
      const suggestion = {
        text: `${routePrefix}_path`,
        snippet: buildSnippet(routePrefix, params),
        type: 'function',
        rightLabel: url,
